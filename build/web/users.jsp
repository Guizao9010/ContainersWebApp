<%-- 
    Document   : users.jsp
    Created on : 27 de jul. de 2023, 14:15:52
    Author     : Guilherme
--%>
                ...
                <div v-else>
                    ...
                    <table class="table">
                        ...
                        <tr v-for="item in list" :key="item.rowId">
                            ...
                            <td>{{ item.role }}</td>
                            <td>
                                <button type="button" @click="removeUser(item.rowId)" class="btn btn-danger btn-sm" title="Remover usuário">
                                    Remover
                                </button>
                            </td>
                        </tr>
    ...
    <script>
            const app = Vue.createApp({
                ...
                methods: {
                    ...
                    async loadList() {
                        const data = await this.request("/ContainersWebApp/api/users", "GET");
                        if(data) {
                            this.list= data.list;
                        }
                    },
                    async addUser() {
                        const data = await this.request("/ContainersWebApp/api/users", "POST", {login: this.newLogin, name: this.newName, role: this.newRole, password: this.newPassword});
                        if(data) {
                            this.newRole = 'USER'; 
                            this.newLogin = ''; 
                            this.newName = ''; 
                    ...
                    async removeUser(id) {
                        const data = await this.request("/ContainersWebApp/api/users?id="+id, "DELETE");
                        if(data) {
                            await this.loadList(); 
                        }
                    }
